...
def navier_stokes_2d(w0, f, visc, T, delta_t=1e-4, record_steps=1):
    ...
    w_h = torch.fft.fft2(w0, norm="backward")
    w_h = torch.stack((w_h.real, w_h.imag), -1)

    # Forcing to Fourier space
    f_h = torch.fft.fft2(f, norm="backward")
    f_h = torch.stack((f_h.real, f_h.imag), -1)
    ...
    for j in tqdm(range(steps)):
        ...
        q = torch.fft.irfft2(torch.view_as_complex(q), s=(N, N), norm="backward")
        ...
        v = torch.fft.irfft2(torch.view_as_complex(v), s=(N, N), norm="backward")
        ...
        w_x = torch.fft.irfft2(torch.view_as_complex(w_x), s=(N, N), norm="backward")
        ...
        w_y = torch.fft.irfft2(torch.view_as_complex(w_y), s=(N, N), norm="backward")

        # Non-linear term (u.grad(w)): compute in physical space then back to Fourier space
        F_h = torch.fft.fft2(q * w_x + v * w_y, norm="backward")
        F_h = torch.stack((F_h.real, F_h.imag), -1)
        ...
        if (j + 1) % record_time == 0:
            ...
            w = torch.fft.irfft2(torch.view_as_complex(w_h), s=(N, N), norm="backward")
