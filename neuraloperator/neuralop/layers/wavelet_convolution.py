...
class WaveConv1d(nn.Module):
    def __init__(
        ...
        self.level = level
        if np.isscalar(size) or len(size) == 1:
            try:
                self.size = size[0]
            except:
                self.size = size
        ...
        self.weights_approx = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels, out_channels, self.effective_modes, dtype=torch.cfloat
            )
        )
    ...
    def spectralconv(self, waves, weights):
        ...
        conv_out[:, :, : self.effective_modes] = self.mul1d(
            xw[:, :, : self.effective_modes], weights
        )
    ...
    def forward(self, x):
        ...
        if x.shape[-1] > self.size:
            ...
            dwt = DWT1D(wave=self.wavelet, J=self.level + factor, mode=self.mode).to(
                x.device
            )
            x_ft, x_coeff = dwt(x)
...
class WaveConv2d(nn.Module):
    def __init__(
        self, in_channels, out_channels, level, size, wavelet, mode="symmetric", omega=8
    ):
        ...
        if isinstance(size, tuple):
            if len(size) != 2:
                raise Exception(
                    "size: WaveConv2dCwt accepts the size of 2D signal in list with 2 elements"
                )
            else:
                self.size = size
        else:
        ...
        # Parameter initilization
        self.scale = 1 / (in_channels * out_channels)
        self.weights_a1 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
                self.effective_modes_x,
                self.effective_modes_x,
                dtype=torch.cfloat,
            )
        )
        self.weights_a2 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
    ...
    def spectralconv(self, waves, weights1, weights2):
        ...
        # Perform Element-wise multiplication in spectral doamin
        conv_out[:, :, : self.effective_modes_x, : self.effective_modes_y] = self.mul2d(
            xw[:, :, : self.effective_modes_x, : self.effective_modes_y], weights1
        )
        conv_out[:, :, -self.effective_modes_x :, : self.effective_modes_y] = (
            self.mul2d(
                xw[:, :, -self.effective_modes_x :, : self.effective_modes_y], weights2
            )
        )
    ...
    def forward(self, x):
        ...
        if x.shape[-1] > self.size[-1]:
            ...
            # Compute single tree Discrete Wavelet coefficients using some wavelet
            dwt = DWT(J=self.level + factor, mode=self.mode, wave=self.wavelet).to(
                x.device
            )
            x_ft, x_coeff = dwt(x)
        ...
        out_coeff[-1][:, :, 0, :, :] = self.spectralconv(
            x_coeff[-1][:, :, 0, :, :].clone(), self.weights_h1, self.weights_h2
        )
        out_coeff[-1][:, :, 1, :, :] = self.spectralconv(
            x_coeff[-1][:, :, 1, :, :].clone(), self.weights_v1, self.weights_v2
        )
        out_coeff[-1][:, :, 2, :, :] = self.spectralconv(
            x_coeff[-1][:, :, 2, :, :].clone(), self.weights_d1, self.weights_d2
        )
...
class WaveConv2dCwt(nn.Module):
    def __init__(
        self, in_channels, out_channels, level, size, wavelet1, wavelet2, omega=8
    ):
        ...
        dummy_data = torch.randn(1, 1, *self.size)
        dwt_ = DTCWTForward(
            J=self.level, biort=self.wavelet_level1, qshift=self.wavelet_level2
        )
        ...
        self.weights_15r1 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
                self.effective_modes_xx,
                self.effective_modes_yy,
                dtype=torch.cfloat,
            )
        )
        self.weights_15r2 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_15c1 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_15c2 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_45r1 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_45r2 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_45c1 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_45c2 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_75r1 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_75r2 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_75c1 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_75c2 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_105r1 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_105r2 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_105c1 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_105c2 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_135r1 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_135r2 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_135c1 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_135c2 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_165r1 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_165r2 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_165c1 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_165c2 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
    ...
    def spectralconv(self, waves, weights1, weights2):
        ...
        # Perform Element-wise multiplication in spectral doamin
        conv_out[:, :, :modes1, :modes2] = self.mul2d(
            xw[:, :, :modes1, :modes2], weights1
        )
        conv_out[:, :, -modes1:, :modes2] = self.mul2d(
            xw[:, :, -modes1:, :modes2], weights2
        )
        return torch.fft.irfft2(conv_out, s=(waves.shape[-2], waves.shape[-1]))
    ...
    def forward(self, x):
        ...
        # Multiply the final detailed wavelet coefficients
        out_coeff[-1][:, :, 0, :, :, 0] = self.spectralconv(
            x_coeff[-1][:, :, 0, :, :, 0].clone(), self.weights_15r1, self.weights_15r2
        )
        out_coeff[-1][:, :, 0, :, :, 1] = self.spectralconv(
            x_coeff[-1][:, :, 0, :, :, 1].clone(), self.weights_15c1, self.weights_15c2
        )
        out_coeff[-1][:, :, 1, :, :, 0] = self.spectralconv(
            x_coeff[-1][:, :, 1, :, :, 0].clone(), self.weights_45r1, self.weights_45r2
        )
        out_coeff[-1][:, :, 1, :, :, 1] = self.spectralconv(
            x_coeff[-1][:, :, 1, :, :, 1].clone(), self.weights_45c1, self.weights_45c2
        )
        out_coeff[-1][:, :, 2, :, :, 0] = self.spectralconv(
            x_coeff[-1][:, :, 2, :, :, 0].clone(), self.weights_75r1, self.weights_75r2
        )
        out_coeff[-1][:, :, 2, :, :, 1] = self.spectralconv(
            x_coeff[-1][:, :, 2, :, :, 1].clone(), self.weights_75c1, self.weights_75c2
        )
        out_coeff[-1][:, :, 3, :, :, 0] = self.spectralconv(
            x_coeff[-1][:, :, 3, :, :, 0].clone(),
            self.weights_105r1,
            self.weights_105r2,
        )
        out_coeff[-1][:, :, 3, :, :, 1] = self.spectralconv(
            x_coeff[-1][:, :, 3, :, :, 1].clone(),
            self.weights_105c1,
            self.weights_105c2,
        )
        out_coeff[-1][:, :, 4, :, :, 0] = self.spectralconv(
            x_coeff[-1][:, :, 4, :, :, 0].clone(),
            self.weights_135r1,
            self.weights_135r2,
        )
        out_coeff[-1][:, :, 4, :, :, 1] = self.spectralconv(
            x_coeff[-1][:, :, 4, :, :, 1].clone(),
            self.weights_135c1,
            self.weights_135c2,
        )
        out_coeff[-1][:, :, 5, :, :, 0] = self.spectralconv(
            x_coeff[-1][:, :, 5, :, :, 0].clone(),
            self.weights_165r1,
            self.weights_165r2,
        )
        out_coeff[-1][:, :, 5, :, :, 1] = self.spectralconv(
            x_coeff[-1][:, :, 5, :, :, 1].clone(),
            self.weights_165c1,
            self.weights_165c2,
        )
        ...
        # Reconstruct the signal
        icwt = DTCWTInverse(biort=self.wavelet_level1, qshift=self.wavelet_level2).to(
            x.device
        )
...
class WaveConv3d(nn.Module):
    def __init__(
        ...
        if isinstance(size, tuple):
            if len(size) != 3:
                raise Exception(
                    "size: WaveConv2dCwt accepts the size of 3D signal in list with 3 elements"
                )
            else:
                self.size = size
        else:
        ...
        mode_data = wavedec3(
            dummy_data, pywt.Wavelet(self.wavelet), level=self.level, mode=self.mode
        )
        ...
        self.weights_aad1 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_aad2 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_aad3 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_aad4 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_ada1 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_ada2 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_ada3 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_ada4 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_add1 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_add2 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_add3 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_add4 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_daa1 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_daa2 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_daa3 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_daa4 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_dad1 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_dad2 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_dad3 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_dad4 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_dda1 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_dda2 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_dda3 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_dda4 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_ddd1 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_ddd2 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_ddd3 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
        ...
        self.weights_ddd4 = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels,
                out_channels,
    ...
    def spectralconv(self, waves, weights1, weights2, weights3, weights4):
        ...
        return torch.fft.irfftn(
            conv_out, s=(waves.shape[-3], waves.shape[-2], waves.shape[-1])
        )
    ...
    def forward(self, x):
        ...
        if x.shape[-1] > self.size[-1]:
            ...
            # Compute single tree Discrete Wavelet coefficients using some wavelet
            x_coeff = wavedec3(
                x, pywt.Wavelet(self.wavelet), level=self.level + factor, mode=self.mode
            )
            ...
            # Compute single tree Discrete Wavelet coefficients using some wavelet
            x_coeff = wavedec3(
                x, pywt.Wavelet(self.wavelet), level=self.level, mode=self.mode
            )
        ...
        x_coeff[1]["aad"] = self.spectralconv(
            x_coeff[1]["aad"].clone(),
            self.weights_aad1,
            self.weights_aad2,
            self.weights_aad3,
        ...
        x_coeff[1]["ada"] = self.spectralconv(
            x_coeff[1]["ada"].clone(),
            self.weights_ada1,
            self.weights_ada2,
            self.weights_ada3,
        ...
        x_coeff[1]["add"] = self.spectralconv(
            x_coeff[1]["add"].clone(),
            self.weights_add1,
            self.weights_add2,
            self.weights_add3,
        ...
        x_coeff[1]["daa"] = self.spectralconv(
            x_coeff[1]["daa"].clone(),
            self.weights_daa1,
            self.weights_daa2,
            self.weights_daa3,
        ...
        x_coeff[1]["dad"] = self.spectralconv(
            x_coeff[1]["dad"].clone(),
            self.weights_dad1,
            self.weights_dad2,
            self.weights_dad3,
        ...
        x_coeff[1]["dda"] = self.spectralconv(
            x_coeff[1]["dda"].clone(),
            self.weights_dda1,
            self.weights_dda2,
            self.weights_dda3,
        ...
        x_coeff[1]["ddd"] = self.spectralconv(
            x_coeff[1]["ddd"].clone(),
            self.weights_ddd1,
            self.weights_ddd2,
            self.weights_ddd3,
        ...
        for jj in range(2, self.level + 1):
            x_coeff[jj] = {
                key: torch.zeros([*x_coeff[jj][key].shape], device=x.device)
